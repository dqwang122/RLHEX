...
def load_trained_gnn(gnn_ckpt, dataset_name, device):
    ...
    model_dict = torch.load(gnn_ckpt, map_location=device)

    name_change = []
    change_dict = {}
    for k, v in model_dict.items():
        if k.startswith('convs.') and k.endswith('.weight') and not k.endswith('lin.weight'):
...
@torch.no_grad()
def load_trained_prediction(gnn_ckpt, dataset_name, device):
    prediction_file = gnn_ckpt
